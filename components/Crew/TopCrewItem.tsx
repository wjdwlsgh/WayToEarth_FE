...
export default function TopCrewItem({ rank, distance, image, highlight, size = highlight ? "lg" : "md", onPress }: Props) {
  ...
  return (
    <TouchableOpacity style={s.wrap} onPress={onPress} activeOpacity={0.8}>
      <View style={[s.imgWrap, { width: dim, height: dim, borderRadius: dim / 2 }, size === "lg" && s.highlight]}>
        {image ? (
          <Image source={image} style={{ width: inner, height: inner, borderRadius: inner / 2 }} />
        ) : (
          <Ionicons name={size === "lg" ? "people" : "people-outline"} size={inner * 0.66} color="#4A90E2" />
        )}
      </View>
